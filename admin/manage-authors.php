<?php
session_start();

// Initialisation des variables de session pour les messages
if (!isset($_SESSION['error'])) {
    $_SESSION['error'] = "";
}
if (!isset($_SESSION['msg'])) {
    $_SESSION['msg'] = "";
}
if (!isset($_SESSION['delmsg'])) {
    $_SESSION['delmsg'] = "";
}

include('includes/config.php');

// Si l'utilisateur n'est plus logué
if (strlen($_SESSION['alogin']) == 0) {
    // On le redirige vers la page de login
    header('location:index.php');
} else {
    // Gestion de la suppression (changement de statut)
    if (isset($_GET['del'])) {
        $id = $_GET['del'];

        // On met à jour le statut à 0 (inactif) au lieu de supprimer
        $sql = "UPDATE tblauthors SET Status=0 WHERE id=:id";
        $query = $dbh->prepare($sql);
        $query->bindParam(':id', $id, PDO::PARAM_STR);

        try {
            $query->execute();
            $_SESSION['delmsg'] = "Auteur désactivé avec succès";
            header('location:manage-authors.php');
        } catch (PDOException $e) {
            $_SESSION['error'] = "Erreur lors de la désactivation";
        }
    }
?>

    <!DOCTYPE html>
    <html lang="FR">

    <head>
        <meta charset="utf-8" />
        <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1" />

        <title>Gestion de bibliothèque en ligne | Gestion des auteurs</title>
        <!-- BOOTSTRAP CORE STYLE  -->
        <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css">
        <!-- FONT AWESOME STYLE  -->
        <link href="assets/css/font-awesome.css" rel="stylesheet" />
        <!-- CUSTOM STYLE  -->
        <link href="assets/css/style.css" rel="stylesheet" />
    </head>

    <body>
        <!------MENU SECTION START-->
        <?php include('includes/header.php'); ?>

        <div class="content-wrapper">
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                        <h4 class="header-line">Gestion des auteurs</h4>
                    </div>
                </div>

                <!-- Messages de succès/erreur -->
                <?php if (isset($_SESSION['error']) && $_SESSION['error'] != "") { ?>
                    <div class="alert alert-danger">
                        <strong>Erreur :</strong>
                        <?php echo htmlentities($_SESSION['error']); ?>
                        <?php $_SESSION['error'] = ""; ?>
                    </div>
                <?php } ?>
                <?php if (isset($_SESSION['msg']) && $_SESSION['msg'] != "") { ?>
                    <div class="alert alert-success">
                        <strong>Succès :</strong>
                        <?php echo htmlentities($_SESSION['msg']); ?>
                        <?php $_SESSION['msg'] = ""; ?>
                    </div>
                <?php } ?>
                <?php if (isset($_SESSION['delmsg']) && $_SESSION['delmsg'] != "") { ?>
                    <div class="alert alert-success">
                        <strong>Succès :</strong>
                        <?php echo htmlentities($_SESSION['delmsg']); ?>
                        <?php $_SESSION['delmsg'] = ""; ?>
                    </div>
                <?php } ?>

                <div class="row">
                    <div class="col-md-12">
                        <div class="panel panel-default">
                            <div class="panel-heading">
                                Liste des Auteurs
                            </div>
                            <div class="panel-body">
                                <div class="table-responsive">
                                    <table class="table table-striped table-bordered table-hover">
                                        <thead>
                                            <tr>
                                                <th>#</th>
                                                <th>Nom</th>
                                                <th>Créé le</th>
                                                <th>Mise à jour le</th>
                                                <th>Action</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <?php
                                            $sql = "SELECT * FROM tblauthors ORDER BY id DESC";
                                            $query = $dbh->prepare($sql);
                                            $query->execute();
                                            $results = $query->fetchAll(PDO::FETCH_OBJ);
                                            $cnt = 1;

                                            if ($query->rowCount() > 0) {
                                                foreach ($results as $result) { ?>
                                                    <tr>
                                                        <td><?php echo htmlentities($cnt); ?></td>
                                                        <td><?php echo htmlentities($result->AuthorName); ?></td>
                                                        <td><?php echo isset($result->creationDate) ? htmlentities($result->creationDate) : 'Non défini'; ?></td>
                                                        <td><?php echo isset($result->UpdationDate) ? htmlentities($result->UpdationDate) : 'Non défini'; ?></td>
                                                        <td>
                                                            <a href="edit-author.php?authorid=<?php echo htmlentities($result->id); ?>">
                                                                <button class="btn btn-primary btn-sm">Éditer</button>
                                                            </a>
                                                            <a href="manage-authors.php?del=<?php echo htmlentities($result->id); ?>"
                                                                onclick="return confirm('Confirmer la désactivation ?');">
                                                                <button class="btn btn-danger btn-sm">Supprimer</button>
                                                            </a>
                                                        </td>
                                                    </tr>
                                            <?php
                                                    $cnt++;
                                                }
                                            } ?>
                                        </tbody>
                                    </table>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <?php include('includes/footer.php'); ?>
        <!-- FOOTER SECTION END-->
        <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js"></script>
        <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js"></script>
    </body>

    </html>
<?php } ?>